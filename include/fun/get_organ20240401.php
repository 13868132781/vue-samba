...
function sql_for_orgs($lj,$zd,$in){
	global $_HLCPHP;
	if(!$in)
		$in='in';
	if($_HLCPHP['global']['orgs']==''){
		return "";
	}else{
		return $lj.' '.$zd." ".$in." (".trim($_HLCPHP['global']['orgs'],',').")";
	}
}
...
function check_for_orid($orid){ 
	...
	if($_HLCPHP['global']['orgs']==''){
		...
		if(strstr($_HLCPHP['global']['orgs'],','.$orid.',')){
			return "true";
		}else{
			return "";
		}
...
权限（orgs），可以为'-'，表全部，或者是节点集合如 ',0,3,6,7,'，每个节点，表示该节点下的设备、用户、管理员，不包括子节点，子节点要自己包含进去
