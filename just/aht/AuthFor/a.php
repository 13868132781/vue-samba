<?php
	...
	function echoOut($code,$msg,$vars=[],$logtype=0,$args=[],$msghz=''){
		...
		if($logtype==1){
			...
			->insert([
				...
				'clientip' => $args['nac'],
				'callingip' => $args['nac'],
				'Reply_Message' => ($msghz?:$msg)
			]);
			
			//app\syslog\syslog::send('radlogin',$args['user'].' '.$args['nas'].' '.$args['nac'].' '.$reply.' '.$msg);
			...
			app\syslog\syslog::send('radoper',$args['user'].' '.$args['nas'].' '.$args['nac'].' '.$args['cmd'].' '.($code?'deny':'allow'));
