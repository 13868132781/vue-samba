...
""" 
    ...
    def do_cd(self, s):
        ...
        if len(self.__outputBuffer.strip('\r\n')) > 0:
            ...
            self.__pwd = ntpath.normpath(ntpath.join(self.__pwd, s.decode(sys.stdin.encoding and sys.stdin.encoding or "UTF-8" )))
            self.execute_remote('cd ')
            self.__pwd = self.__outputBuffer.strip('\r\n').decode(CODEC)
            self.prompt = unicode(self.__pwd + '>').encode(sys.stdout.encoding and sys.stdout.encoding or "UTF-8")
            self.__outputBuffer = ''
    ...
    def execute_remote(self, data):
        ...
        self.__win32Process.Create(command.decode(sys.stdin.encoding and sys.stdin.encoding or "UTF-8"), self.__pwd, None)
        self.get_output()
