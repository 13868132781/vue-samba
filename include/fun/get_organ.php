...
function show_organs($a){
	...
	$qury="select * from ".sql_join_organ("radius.organ","or_id")." a where or_fid = '".$a['id']."' ".$limitorgans." ".$tagselect." order by or_order";
...
function get_or_far($db,$id){
	...
	if($qury_rows!=0 and $qury_line['or_fid']!='0')
...
function sql_for_orgs($lj,$zd,$in='in'){
	//该函数已经废弃
	//因为如果选中id太多，sql语句没法执行
	//用下面的sql_join_organ
	global $_HLCPHP;
	return "";
}

function sql_join_organ($table,$col,$all=true){
	global $_HLCPHP;
	if($_HLCPHP['global']['orgcount']=='0'){
		return $table;
	}else{
		$userid = $_HLCPHP['global']['acid'];
		$allsql="";
		if(!$all){
			$allsql=" and  uo_select=1 ";
		}
		return "(select * from ".$table." xxxx left join (select * from system.sys_user_organ where uo_usid='".$userid."' ".$allsql.") yyyy on xxxx.".$col."=yyyy.uo_orid where uoid!='')";
	}
}

$loginadminorglist="notget";//所选中的机构及其父级别机构的列表
function check_for_orid($orid){//目前没用到，改用那面拼接sql语句的方式了
	global $_HLCPHP, $loginadminorglist,$mysite;
	if($loginadminorglist=='notget'){
		if($_HLCPHP['global']['orgcount']=='0'){//为0，表示全部权限
			$loginadminorglist='';
		}else{
			$userid = $_HLCPHP['global']['acid'];
			$qury_sql="select * from system.sys_user_organ where uo_usid='".$userid."'";
			$qury_obj=mysql_query($qury_sql, $mysite) or die(mysql_error());
			$orgstr = "";
			while($qury_row=mysql_fetch_assoc($qury_obj)){
				if($orgstr!=''){
					$orgstr.=",";
				}
				$orgstr.=$qury_row['uo_orid'];
			}
			$loginadminorglist = $orgstr;
		}
	}
	if($loginadminorglist==''){
		return "true";
	}else{
		if(strstr(",".$loginadminorglist.",",','.$orid.',')){
			return "true";
		}else{
			return "";
		}
