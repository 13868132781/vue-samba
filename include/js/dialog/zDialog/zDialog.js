...
Dialog.setBgDivSize = function () {
	...
	if(Dialog.bgDiv){
			if(isIE6){
				...
				var clientHeight=window.document.documentElement.clientHeight?window.document.documentElement.clientHeight:window.document.body.offsetHeight;
				if(bd.scrollHeight > clientHeight)
					Dialog.bgDiv.style.height = bd.scrollHeight + "px";
				else
					Dialog.bgDiv.style.height = clientHeight + "px";
			}
		}
};
...
Dialog.prototype.create = function () {
	...
	var pluswidth=1+1;
	var plusheight=25 + 1 + (this.ShowButtonRow ? 28 : 0) + (this.ShowMessageRow ? 50 : 0);
    if (!this.Width || (this.Width+pluswidth)>bd.clientWidth){
		if(this.wpercent){
			this.Width=bd.clientWidth*this.wpercent-pluswidth;
		}else{
			this.Width = bd.clientWidth - Math.round(this.leftpx?(this.leftpx*2):26 )-pluswidth;
		}
		
	}
	if (!this.Height || (this.Height+plusheight)>bd.clientHeight){
		if(this.hpercent){
			this.Height=bd.clientHeight*this.wpercent-plusheight;
		}else{
			this.Height = bd.clientHeight - Math.round(this.toppx?(this.toppx*2):46)-plusheight ;
		}
    ...
    var DialogDivWidth = this.Width + 1 + 1;
    var DialogDivHeight = this.Height + 25 + 1 + (this.ShowButtonRow ? 40 : 0) + (this.ShowMessageRow ? 50 : 0);
	//DialogDivWidth没有实际用处，只是下面对比，确定是否比窗口大
    if (this.leftpx || DialogDivWidth > bd.clientWidth) this.Width = Math.round(bd.clientWidth - 26 );
    if (this.toppx || DialogDivHeight > bd.clientHeight) this.Height = Math.round(bd.clientHeight - 46 - (this.ShowButtonRow ? 40 : 0) - (this.ShowMessageRow ? 50 : 0));
	*/
	
	
    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 2) + '" cellspacing="0" cellpadding="0"  onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;" class="pop_table">\
	<tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + ';">\
		<td height="20" class="pop_table_titletd">\
	  		<div style=" float: left;">\
	  			<span id="_Title_' + this.ID + '">' + this.Title + '</span>\
	  		</div>\
        	<div align="right" onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="" onmouseover="" style=" cursor: pointer; float: right; width: 30px; ' + (this.ShowCloseButton ? "" : "display:none;") + '">\
				<span >关闭</span>\
			</div>\
	  	</td>\
	</tr>\
			...
			<table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          		<tr id="_MessageRow_' + this.ID + '" style="' + (this.ShowMessageRow ? "" : "display:none") + '">\
            		<td valign="top" height="50">\
						<table width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#eaece9 ;" id="_MessageTable_' + this.ID + '">\
                			<tr>\
                  				...
                  				<td align="left" style="line-height: 16px;">\
									<div id="_MessageTitle_' + this.ID + '" style="font-weight:bold">' + this.MessageTitle + '</div>\
                   					<div id="_Message_' + this.ID + '">' + this.Message + '</div>\
								</td>\
                			</tr>\
              			</table>\
					</td>\
          		...
          		<tr>\
            		<td valign="top" align="center">\
						<div id="_Container_' + this.ID + '" style="position: relative; width: ' + this.Width + 'px; height: ' + this.Height + 'px;">\
	...
	' + (function (obj) {
        ...
        return "";
          		...
          		<tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            		<td>\
						<div id="_DialogButtons_' + this.ID + '" class="pop_table_btm_div">\
                			<a href="javascript:void(0);" class="main_btn_text"  onclick="v=Dialog.getInstance(\'' + this.ID + '\').innerFrame.contentWindow.document.getElementById(\'submit\');if(v!=null)v.click();" id="_ButtonOK_' + this.ID + '" style="margin-right:5px;padding-left:15px;padding-right:15px"> 确 定 </a>\
                			<a href="javascript:void(0);" class="main_btn_text"  onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '" style="margin-right:5px;padding-left:15px;padding-right:15px"> 取 消 </a>\
              			</div>\
					</td>\
          		</tr>\
        	</table>\
...
Dialog.prototype.close = function () {
    ...
    if (topWin.Dialog._Array.length > 0) {
        ...
        var topWinBody = topDoc.getElementsByTagName(topDoc.compatMode == "BackCompat" ? "BODY" : "HTML")[0];
        if (topWinBody.styleOverflow != undefined) topWinBody.style.overflow = topWinBody.styleOverflow; //恢复滚动条
    }
    if (isIE) {
		/*****释放引用，以便浏览器回收内存**/
...
Dialog.getBgdiv = function () {
    ...
    if (!bgdiv) {
		...
		var bgIframeMask = '<div id="_DialogBGMask" style="position:absolute;background-color:#999;opacity:0.6;filter:alpha(opacity=60);width:100%;height:100%;"></div>';
...
Dialog.close_all = function () {
	for(var i=0;i<topWin.Dialog._Array.length;i++){
        var diag = topWin.Dialog._Array[i];
        diag.close();
    }
}

Dialog.cancel = function (id) {
	var diag = Dialog.getInstance(id);
	if(diag){
		diag.cancelButton.onclick.apply(diag.cancelButton, []);
	}
};
Dialog.cancel_last = function () {
    if (topWin.Dialog._Array.length > 0) {
        var diag = topWin.Dialog._Array[topWin.Dialog._Array.length - 1];
        diag.cancelButton.onclick.apply(diag.cancelButton, []);
    }
};
Dialog.cancel_all = function () {
	for(var i=0;i<topWin.Dialog._Array.length;i++){
        var diag = topWin.Dialog._Array[i];
        diag.cancelButton.onclick.apply(diag.cancelButton, []);
    }
